...
/**
 * функция, реализующая метод Гаусса-Зейделя для решения СЛАУ
...
template <typename T>
std::vector<T>
GaussSeidel(const CSR<T> &A, const std::vector<T> &b, const std::vector<T> &init_res, const T &tolerance) {
#ifndef NDEBUG
    if (A.sizeH() != A.sizeW()) {
    ...
    // алгоритм описан в docs -> Системы уравнений. Лекция 2. Базовые методы.pdf стр. 30
    while (norm(A * res - b, NormType::ThirdNorm) > tolerance) {
        for (int i = 0; i < b.size(); ++i) {
            ...
            for (int k = skip; k < skip + count; ++k) {
                if (i != A.cols_[k]) sum += A.values_[k] * res[i];
                else continue;
            }
            res[i] = (b[i] - sum) / A(i, i);
        }
